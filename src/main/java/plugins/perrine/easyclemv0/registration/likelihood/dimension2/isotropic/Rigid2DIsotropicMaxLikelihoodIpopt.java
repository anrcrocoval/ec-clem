...
public class Rigid2DIsotropicMaxLikelihoodIpopt extends Ipopt {
    ...
    protected boolean eval_grad_f(int n, double[] x, boolean new_x, double[] grad_f) {
        ...
        grad_f[0] = derivativeStructure.getPartialDerivative(1, 0, 0, 0);
        grad_f[1] = derivativeStructure.getPartialDerivative(0, 1, 0, 0);
        grad_f[2] = derivativeStructure.getPartialDerivative(0, 0, 1, 0);
        grad_f[3] = derivativeStructure.getPartialDerivative(0, 0, 0, 1);
        return true;
    }
    ...
    protected boolean eval_h(int n, double[] x, boolean new_x, double obj_factor, int m, double[] lambda, boolean new_lambda, int nele_hess, int[] iRow, int[] jCol, double[] values) {
        ...
        if(values == null) {
            ...
            DerivativeStructure derivativeStructure = getDerivativeStructure(x);
            values[0] = derivativeStructure.getPartialDerivative(2, 0, 0, 0) * obj_factor;
            values[1] = derivativeStructure.getPartialDerivative(1, 1, 0, 0) * obj_factor;
            values[2] = derivativeStructure.getPartialDerivative(0, 2, 0, 0) * obj_factor;
            values[3] = derivativeStructure.getPartialDerivative(1, 0, 1, 0) * obj_factor;
            values[4] = derivativeStructure.getPartialDerivative(0, 1, 1, 0) * obj_factor;
            values[5] = derivativeStructure.getPartialDerivative(0, 0, 2, 0) * obj_factor;
            values[6] = derivativeStructure.getPartialDerivative(1, 0, 0, 1) * obj_factor;
            values[7] = derivativeStructure.getPartialDerivative(0, 1, 0, 1) * obj_factor;
            values[8] = derivativeStructure.getPartialDerivative(0, 0, 1, 1) * obj_factor;
            values[9] = derivativeStructure.getPartialDerivative(0, 0, 0, 2) * obj_factor;
    ...
    private DerivativeStructure getDerivativeStructure(double[] point) {
        DerivativeStructure tx = new DerivativeStructure(4, 2, 0, point[0]);
        DerivativeStructure ty = new DerivativeStructure(4, 2, 1, point[1]);
        DerivativeStructure theta = new DerivativeStructure(4, 2, 2, point[2]);
        DerivativeStructure lambda_11 = new DerivativeStructure(4, 2, 3, point[3]);
